...
class cpu:
    ...
    def dump_memory(self):
        ...
        for i in range(self.max_memory):
            if i not in self.pmemory and i in self.memory and self.memory[i] != 0:
                print('  m[{}] {}'.format(i, self.memory[i]))
    ...
    def mdump(self, index):
        print('m[{}] {}'.format(index, self.memory[index]))
        return
        ...
        if tmp[0] == '$':
            ...
            if len(s) == 1:
                ...
                if len(t) == 1:
                    ...
                    print('mov: bad argument [{}]'.format(tmp))
                    exit(1)
                    return
            else:
                print('mov: bad argument [{}]'.format(tmp))
    ...
    def load(self, infile, loadaddr):
        ...
        for line in fd:
            ...
            if cline[0] != '.':
                ...
                if self.verbose:
                    print('pc:{} LOADING {:20s} --> {}'.format(pc, self.pmemory[pc], self.memory[pc]))
    ...
    def print_headers(self, procs):
        ...
        if len(self.memtrace) > 0:
            for m in self.memtrace:
                if m[0].isdigit():
                    print('{:5d}'.format(int(m)), end=' ')
                else:
                    zassert(m in self.vars, 'Traced variable %s not declared' % m)
                    print('{:5s}'.format(m), end=' ')
            print(' ', end='')
        if len(self.regtrace) > 0:
            for r in self.regtrace:
                print('{:5s}'.format(self.get_regname(r)), end=' ')
            print(' ', end='')
        if cctrace:
            print('>= >  <= <  != ==', end=' ')
        ...
        for i in range(procs.getnum()):
            print('       Thread {}        '.format(i), end='')
        print()
        return

    def print_trace(self, newline):
        if self.printstats:
            print('{:6d}'.format(self.icount), end=' ')
        if len(self.memtrace) > 0:
            for m in self.memtrace:
                if self.compute:
                    if m[0].isdigit():
                        print('{:5d}'.format(self.memory[int(m)]), end=' ')
                    else:
                        zassert(m in self.vars, 'Traced variable %s not declared' % m)
                        print('{:5d}'.format(self.memory[self.vars[m]]), end=' ')
                else:
                    print('{:5s}'.format('?'), end=' ')
            print(' ', end='')
        if len(self.regtrace) > 0:
            for r in self.regtrace:
                if self.compute:
                    print('{:5d}'.format(self.registers[r]), end=' ')
                else:
                    print('{:5s}'.format('?'), end=' ')
            print(' ', end='')
        if cctrace:
            for c in self.condlist:
                if self.compute:
                    ...
                    print('? ', end='')
        if (len(self.memtrace) > 0 or len(self.regtrace) > 0 or cctrace) and newline:
            print()
        return
    ...
    def run(self, procs, intfreq, intrand):
        ...
        while True:
            ...
            if interrupt == 0 or rc == -2:
                ...
                if not procs.ismanual() or (procs.ismanual() and curr != next):
                    self.print_trace(False)
                    for i in range(procs.getnum()):
                        print('------ Interrupt ------ ', end='')
                    print()
...
class proclist:
    ...
    def finalize(self, procsched):
        ...
        for i in range(len(procsched)):
            p = int(procsched[i])
            if p >= self.getnum():
                print('bad schedule: cannot include a thread that does not exist ({})'.format(p))
                exit(1)
...
if printstats:
    print()
    print('STATS:: Instructions    {}'.format(ic))
    print('STATS:: Emulation Rate  {:.2f} kinst/sec'.format(float(ic) / float(t2 - t1) / 1000.0))
